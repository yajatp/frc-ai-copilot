...
function updateAntigravityConfig(executablePath, javaHome) {
    console.log('⚙️ Updating Antigravity mcp_config.json...');
    
    const antigravityConfigDir = path.join(os.homedir(), '.gemini', 'antigravity-ide');
    ...
    if (fs.existsSync(configPath)) {
        try {
            ...
            console.error('❌ Failed to parse existing mcp_config.json. Creating a fresh configuration backup...');
    ...
    try {
        ...
        console.error('❌ Failed to write mcp_config.json: ', e.message);
        process.exit(1);
    }
}

function main() {
    console.log('🚀 Starting Antigravity Integration Setup');
    const { executablePath, javaHome } = buildServer();
    updateAntigravityConfig(executablePath, javaHome);
    console.log('\\n🎉 Setup complete! You can now use the FRC AI Copilot inside Antigravity IDE.');
}
